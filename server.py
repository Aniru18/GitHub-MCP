...
Key upgrades over v1:
  ...
  - Per-request GitHub PAT token: each caller passes their own token via the
    `github_token` tool argument.  The server never uses a hard-coded token.
  - HTTP transport (Starlette/uvicorn) so the server can be deployed to
    Vercel, FastMCP Cloud, or any container platform.

Run locally:
    pip install "mcp[cli]" PyGithub anyio httpx starlette uvicorn fastmcp
    python server.py                        # listens on 0.0.0.0:8081
    PORT=9000 python server.py              # custom port

Deploy to Vercel:
    - Add api/index.py that imports `app = mcp.http_app()`
    - vercel --prod

Clients must pass their GitHub PAT via the `github_token` parameter on
every tool call.  For private repos or higher rate-limits a fine-grained
PAT with `repo` / `read:org` scopes is recommended.
"""
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "and run deep structural analysis across any public (or private, with a "
        "suitable PAT) GitHub repository."
    ),
)
...
# ── Tool 11: clone_repo (disabled on Vercel) ─────────────────────────────────
...
async def clone_repo(
    ...
    ⚠️  This tool is disabled in the Vercel deployment because Vercel's
    serverless runtime has no `git` binary and a read-only filesystem.
    Run the server locally (python server.py) to use this tool.
    ...
    """
    return (
        "⚠️  clone_repo is not available in the Vercel deployment.\n"
        "All other 12 tools work normally.\n"
        "To clone repositories, run the server locally: python server.py"
    )
...
#.........uncomment this code and comment the abve code if you want to add this cluade config...........
